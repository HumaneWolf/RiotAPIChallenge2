...
echo "Loading...\n";
//EUNE
$matches = file_get_contents("riot_dataset/5.14/RANKED_SOLO/EUNE.json");
$matches = json_decode($matches, TRUE);

foreach ($matches as $m) {
	eunestart:
	echo "Loading EUNE " . $m . "\n";
	$json = @file_get_contents("https://eune.api.pvp.net/api/lol/eune/v2.2/match/" . $m . "?api_key=" . $api['key']);
		...
		foreach ($json['participants'] as $p) {
			...
			if (isset($p['stats']['item0'])) {
				...
				if ($item == 1058 || $item == 3089 || $item == 3157 || $item == 3285 || $item == 3116 || $item == 3003 || $item == 3040 || $item == 3027 || $item == 3136 || $item == 3151 || $item == 3135 || $item == 3115 || $item == 3152 || $item == 3165 || $item == 3174 || $item == 1026) { //is a changed ap item
					if ($s = $sql->prepare("INSERT INTO items_514 (champ, item, win, matchid, server) VALUES (?, ?, ?, ?, ?)")) {
			...
			if ($s = $sql->prepare("INSERT INTO games_514 (matchid, champ, win, server, other, dmg, otherap, appart) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
